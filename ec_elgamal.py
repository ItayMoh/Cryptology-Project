...
class Curve(ABC):
    ...
    def encode_point(self, plaintext: bytes) -> Point:
        """
        Encode 'plaintext' as an (x, y) point on the curve by:
          1) Prefixing the plaintext with its length (1 byte).
          2) Treating that as a big-endian integer x.
          3) Attempting to find y via self.compute_y(x).
          4) If no valid y, append one random byte to 'plaintext_with_length' and retry.
        """
        if not plaintext:
            raise ValueError("Plaintext cannot be empty.")

        # Prefix length as the first byte
        plaintext_with_length = len(plaintext).to_bytes(1, byteorder="big") + plaintext

        while True:
            # Interpret the byte sequence as an integer (x-coordinate)
            x = int.from_bytes(plaintext_with_length, "big")

            try:
                y = self.compute_y(x)
                # If compute_y did not raise, y is valid (nonzero), so return the point
                return Point(x, y, self)

            except ValueError:
                # Means we did not find a valid square root for this x => add random byte and retry
                plaintext_with_length += urandom(1)
    def decode_point(self, M: Point) -> bytes:
        """
            Decode the point M back into plaintext by reversing the encoding scheme:
              - The first byte of x (in big-endian) is the length 'plaintext_len'
              - The next 'plaintext_len' bytes are the actual plaintext
            """
        if M.is_at_infinity():
            raise ValueError("Point is at infinity and cannot be decoded.")

        x = M.x  # Extract x-coordinate as an integer
        byte_len = int_length_in_byte(x)  # Number of bytes required to represent x in big-endian

        # Extract the plaintext length from the top (first) byte
        # Example: if x is 0x0548656c6c6f in hex => that's 6 bytes => top byte is 0x05 => length=5
        plaintext_len = (x >> ((byte_len - 1) * 8)) & 0xff

        if plaintext_len <= 0 or plaintext_len > (byte_len - 1):
            raise ValueError(f"Invalid plaintext length: {plaintext_len}")

        # Extract the plaintext data from the next plaintext_len bytes
        # Shift out the top byte (which is the length), then mask to get only plaintext_len bytes
        shift_amount = (byte_len - plaintext_len - 1) * 8
        mask = (1 << (plaintext_len * 8)) - 1
        plaintext_int = (x >> shift_amount) & mask

        # Convert to bytes
        return plaintext_int.to_bytes(plaintext_len, byteorder="big")
...
class MontgomeryCurve(Curve):
    ...
    def compute_y(self, x: int) -> int:
        """
        Compute y for a given x on this Montgomery curve, i.e. solve
             b * y^2 = x^3 + a * x^2 + x   (mod p)
        Return y if a solution exists, or raise ValueError otherwise.
        """
        right = (x * x * x + self.a * x * x + x) % self.p
        inv_b = modinv(self.b, self.p)
        right = (right * inv_b) % self.p
        y = modsqrt(right, self.p)
        # If modsqrt returns 0, it means 'right' is not a quadratic residue mod p
        if y == 0:
            raise ValueError("No square root found for x on this curve.")
        return y
...
def ec_elgamal_encrypt(public_key: Point, message: bytes, curve: Curve25519) -> (Point, Point):
    """
        Encrypt 'message' by splitting into chunks (≤31 bytes each),
        encoding each chunk to a point, and producing a list of (c1, c2) pairs.
        """
    # Maximum chunk size is 31 bytes so it can be encoded in x
    # along with the 1-byte length prefix
    MAX_CHUNK_LEN = 31

    # Split the message into ≤31-byte chunks
    chunks = [
        message[i: i + MAX_CHUNK_LEN]
        for i in range(0, len(message), MAX_CHUNK_LEN)
    ]

    ciphertext_chunks = []
    for chunk in chunks:
        # Encode chunk into a point
        encoded_message_point = curve.encode_point(chunk)
        # Generate random k
        k = random.randint(1, curve.n - 1)
        # c1 = k * G
        c1 = k * curve.G
        # c2 = encoded_message_point + k * public_key
        c2 = encoded_message_point + (k * public_key)
        ciphertext_chunks.append((c1, c2))

    return ciphertext_chunks

def ec_elgamal_decrypt(private_key: int, ciphertext_chunks, curve: Curve25519) -> bytes:
    """
    Decrypt each (c1, c2) pair, decode the chunk, and reassemble the full plaintext.
    """
    plaintext = b""
    for (c1, c2) in ciphertext_chunks:
        # Recover the encoded message point
        decoded_point = c2 - (private_key * c1)
        # Decode to plaintext chunk
        chunk_plaintext = curve.decode_point(decoded_point)
        # Concatenate
